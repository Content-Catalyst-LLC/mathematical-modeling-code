...
fn main() {
    ...
    for (x, order) in cases {
        ...
        let reference = x.exp();
        let abs_err = (reference - approximation).abs();
        let rel_err = abs_err / reference.abs();
        let warning = if x.abs() <= 2.0 { "" } else { "Evaluation is far from the expansion center; review local validity." };
        println!("Maclaurin truncation,exp(x),0.0,{:.12},{},{:.12},{:.12},{:.12},{:.12},{}", x, order, approximation, reference, abs_err, rel_err, warning);
    }
}
