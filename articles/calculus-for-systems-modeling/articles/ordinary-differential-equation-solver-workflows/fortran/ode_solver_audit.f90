program ode_solver_audit
  implicit none
  integer :: step, steps
  real(8) :: y0, k, h, stop_time, t, exact, y
  ...
  print '(A)', 'step time solver_value exact_value absolute_error step_size'
  do step = 0, steps
    t = dble(step) * h
    exact = y0 * exp(-k * t)
    print '(I6,5F16.8)', step, t, y, exact, abs(y - exact), h
    y = rk4_next(t, y, h, k)
  end do
