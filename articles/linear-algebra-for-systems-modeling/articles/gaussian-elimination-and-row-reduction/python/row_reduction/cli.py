    ...
    for column in range(column_count):
        ...
        for row in range(row_count):
            if row != pivot_row:
                ...
                rows[row] = [current - factor * pivot_current for current, pivot_current in zip(rows[row], rows[pivot_row])]
        pivot_columns.append(column)
        pivot_row += 1
        if pivot_row == row_count:
            break
    cleaned = [[0.0 if abs(value) < tolerance else round(value, 10) for value in row] for row in rows]
    return cleaned, pivot_columns

def rank_from_rref(reduced: Matrix, coefficient_columns: int, tolerance: float = 1e-10) -> int:
    return sum(1 for row in reduced if any(abs(value) > tolerance for value in row[:coefficient_columns]))
...
def build_audit() -> RowReductionAudit:
    augmented = [[1.0, 1.0, 0.0, 100.0], [0.0, 1.0, 1.0, 80.0], [1.0, 0.0, 1.0, 90.0]]
    tolerance = 1e-10
    unknown_count = 3
    reduced, pivots = rref(augmented, tolerance=tolerance)
    coefficient_rank = rank_from_rref(reduced, unknown_count, tolerance)
    augmented_rank = augmented_rank_from_rref(reduced, tolerance)
    ...
    return RowReductionAudit(
        ...
        len(augmented),
        unknown_count,
        ",".join(str(column) for column in variable_pivots),
        coefficient_rank,
        augmented_rank,
        consistent,
        behavior,
        tolerance,
        "Row reduction reveals algebraic structure, but the computed solution still requires feasibility checks, unit review, and validation against original constraints.",
    )
...
def write_outputs(output_dir: Path) -> None:
    ...
    with (output_dir / "tables" / "row_reduction_audit.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(row.keys()))
        writer.writeheader()
        writer.writerow(row)
    (output_dir / "json" / "row_reduction_audit.json").write_text(json.dumps(row, indent=2, sort_keys=True), encoding="utf-8")
