...
def write_outputs(output_dir: Path) -> None:
    ...
    with (output_dir / "tables" / "advanced_state_vector_audit.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=list(rows[0].keys()))
        writer.writeheader()
        writer.writerows(rows)
    (output_dir / "json" / "advanced_state_vector_audit.json").write_text(json.dumps(rows, indent=2, sort_keys=True), encoding="utf-8")
    ...
    (output_dir / "reports" / "advanced_state_vector_audit.md").write_text("\n".join(lines), encoding="utf-8")
