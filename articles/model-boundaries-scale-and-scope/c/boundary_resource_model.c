...
int main(void) {
    ...
    const double policy_savings = 1.5;
    const double loss_rate = 0.015;
    const int periods = 60;
    double total_shortage = 0.0;

    printf("period,stock,inflow,effective_demand,losses,shortage\n");
    for (int period = 0; period <= periods; ++period) {
        double effective_demand = demand - policy_savings;
        ...
        printf("%d,%.6f,%.6f,%.6f,%.6f,%.6f\n", period, stock, inflow, effective_demand, losses, shortage);
