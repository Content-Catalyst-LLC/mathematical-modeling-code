...
int main(void) {
    ...
    printf("period,stock,inflow,demand,losses,shortage\n");
    for (int period = 0; period <= periods; ++period) {
        double losses = loss_rate * stock;
        double shortage = demand + losses - (stock + inflow);
        if (shortage < 0.0) shortage = 0.0;

        printf("%d,%.6f,%.6f,%.6f,%.6f,%.6f\n", period, stock, inflow, demand, losses, shortage);
        total_shortage += shortage;
        stock = bounded_update(stock, inflow, demand, losses, capacity);
    }
